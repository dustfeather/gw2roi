...
//
// A factory rather than free functions: on Workers there is no module-scope env to read the
// key and base URL from, and closing over one `Config` per invocation keeps every call site
// from having to thread it. The throttle clock lives in the closure for the same reason — it
// is per-run state, not per-isolate state, so a warm isolate cannot start a run believing it
...
// Max known rating per discipline across all account characters. Pure, so it stays a plain
// export rather than a client method.
...
export interface Gw2Client {
  fetchCharacters(): Promise<Character[]>;
  fetchUnlockedRecipeIds(): Promise<Set<number>>;
  fetchAllRecipeIds(): Promise<number[]>;
  fetchRecipes(ids: number[], onPage?: (page: Recipe[]) => Promise<void>): Promise<Recipe[]>;
  fetchItems(ids: number[], onPage?: (page: Item[]) => Promise<void>): Promise<Item[]>;
  fetchHeldMats(chars: Character[]): Promise<Map<number, number>>;
  fetchWalletCoin(): Promise<number | null>;
...
export function createGw2Client(cfg: Config): Gw2Client {
  ...
  const fetchCharacters = (): Promise<Character[]> =>
    getJson<Character[]>("/v2/characters?ids=all", true);

  // Recipe ids the account has explicitly unlocked (excludes AutoLearned).
  const fetchUnlockedRecipeIds = async (): Promise<Set<number>> =>
    new Set(await getJson<number[]>("/v2/account/recipes", true));

  // Every recipe id in the game.
  const fetchAllRecipeIds = (): Promise<number[]> => getJson<number[]>("/v2/recipes", false);

  const fetchRecipes = (
    ids: number[],
    onPage?: (page: Recipe[]) => Promise<void>,
  ): Promise<Recipe[]> => getBulk<Recipe>("/v2/recipes", ids, onPage);
  ...
  const fetchItems = (ids: number[], onPage?: (page: Item[]) => Promise<void>): Promise<Item[]> =>
    getBulk<Item>("/v2/items", ids, onPage);
  ...
  async function fetchHeldMats(chars: Character[]): Promise<Map<number, number>> {
  ...
  async function fetchWalletCoin(): Promise<number | null> {
  ...
  async function fetchTpTransactions(known: Set<number> = new Set()): Promise<TpTxn[]> {
    const out: TpTxn[] = [];
